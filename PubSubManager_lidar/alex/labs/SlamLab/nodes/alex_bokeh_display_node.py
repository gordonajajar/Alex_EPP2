...
# Plotting Costants
FRAMERATE =60
LIDAR_OFFSET_DEGREES = 0 # should match whatever is set in the SLAM node
...
def makeRobotRectangle(robotOriginXmm, robotOriginYmm, robotThetaDegrees=0):
    """
    Generate coordinates for a rectangle representing the robot.

    Args:
        robotOriginXmm (float): X coordinate of robot center in mm.
        robotOriginYmm (float): Y coordinate of robot center in mm.
        robotThetaDegrees (float): Heading in degrees (from north). Default is 0.

    Returns:
        tuple: Two lists containing the X and Y coordinates of the rectangle vertices.
    """
    # Robot dimensions (change as needed)
    ROBOT_WIDTH_MM = 100      # side-to-side
    ROBOT_LENGTH_MM = 150     # front-to-back

    # Calculate half-dimensions
    half_width = ROBOT_WIDTH_MM / 2
    half_length = ROBOT_LENGTH_MM / 2

    # Convert heading to radians and rotate coordinates accordingly
    theta = np.deg2rad(-robotThetaDegrees + 90)  # adjust for Bokeh's system

    def rotate_offset(dx, dy):
        x = robotOriginXmm + dx * np.cos(theta) - dy * np.sin(theta)
        y = robotOriginYmm + dx * np.sin(theta) + dy * np.cos(theta)
        return [x, y]

    # Four corners (clockwise)
    front_left     = rotate_offset(-half_width, -half_length)
    front_right    = rotate_offset(half_width, -half_length)
    back_right     = rotate_offset(half_width, half_length)
    back_left      = rotate_offset(-half_width, half_length)

    # Separate X and Y for Bokeh polygon drawing
    robot_Xs = [front_left[0], front_right[0], back_right[0], back_left[0]]
    robot_Ys = [front_left[1], front_right[1], back_right[1], back_left[1]]
...
def createLidarPlot():
    ...
    # Create a ColumnDataSource to hold Lidar scan points
    source = ColumnDataSource(data=dict(x=[], y=[]))

    # Add scatter plot for Lidar points
    p.scatter(x='x', y='y', source=source, size=3, color="red", alpha=1,
              legend_label="Lidar Scan", marker="circle")
    ...
    # Use rectangle instead of triangle
    robot_Xs, robot_Ys = makeRobotRectangle(0, 0, 0)  # replace with makeRobotTriangle if needed
    p.patch(x=robot_Xs, y=robot_Ys, fill_color="red", line_color="black", alpha=1)
    ...
    for i in range(firstRing_radius, max_distance, ring_interval):
        p.circle(x=0, y=0, radius=i, fill_color=None, line_color="black", line_width=0.5, alpha=0.5)

        # Add distance labels at 45 degrees
        angle = np.deg2rad(45)
        x = i * np.cos(angle)
        y = i* np.sin(angle)
        ...
        p.text(x=[x + x_offset], y=[y + y_offset], text=[f"{i} mm"],
